...
public class jiraRevision {
	...
	public static void main(String[] args) {
		...
		given().log().all().pathParam("id", "10311").header("Content-Type","application/json").body("{\r\n"
				+ "    \"body\":\""+expected+"\",\r\n"
				+ "    \"visibility\":{\r\n"
				+ "        \"type\":\"role\",\r\n"
		...
		given().log().all().pathParam("id", "10311" ).header("X-Atlassian-Token","no-check").filter(session).multiPart("file",new File("jira.txt")).header("Content-Type","multipart/form-data").when().post("/rest/api/2/issue/{id}/attachments");
	
	//get request
		
		String getResponse=
		given().filter(session).queryParam("fields", "comment").pathParam("id","10311" ).when().get("/rest/api/2/issue/{id}").then().log().all().assertThat().statusCode(200).extract().response().asString();
